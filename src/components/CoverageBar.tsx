    ...
    return (
      <div className={`h-full ${bgClass} transition-all duration-500 flex items-center justify-center overflow-hidden`} style={{ width: `${percentage}%` }}>
        {percentage >= 15 ? (
          ...
          <div className={`flex flex-col items-center justify-center leading-none whitespace-nowrap px-0.5 origin-center scale-[0.85] ${textClass}`}>
            <span className={`text-[7px] uppercase tracking-normal font-bold mb-0.5 ${subtitleClass}`}>{label}</span>
            <span className="text-[11px] font-black tracking-tight">{percentage}%</span>
          </div>
        ) : percentage >= 5 ? (
          <span className={`text-[9px] sm:text-[10px] font-bold whitespace-nowrap ${textClass}`}>{percentage}%</span>
        ) : null}
      </div>
    );
  };
