...
import { aggregateNewsBuildTopics, getTimeAgo, calculateBiasDistribution, calculateBlindspot } from '../aggregation.js';
 ...
 */
function findMatchingExistingStory(freshStory, existingStories) {
    ...
    return existingStories.find(s => s.sources.some(src => freshUrls.has(src.link))) ?? null;
}
/**
 ...
 */
function mergeWithExistingSources(freshStory, existingStory) {
    const freshUrls = new Set(freshStory.sources.map(s => s.link));
    ...
    if (droppedSources.length === 0)
        return freshStory; // nimic nou de adăugat
...
async function acquireRefreshLock(redis) {
    const lockId = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
    const result = await redis.set(REFRESH_LOCK_KEY, lockId, { nx: true, ex: REFRESH_LOCK_TTL });
    return result === 'OK' ? lockId : null;
}
async function releaseRefreshLock(redis, lockId) {
    try {
        const currentLock = await redis.get(REFRESH_LOCK_KEY);
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const existingRaw = await redis.get(CACHE_KEY);
        const existingStories = existingRaw ?? [];
        ...
        const carryForwardStories = existingStories
            ...
            if (age > MAX_STORY_AGE_MS)
                return false;
            // Deja inclus în fresh (prin ID sau URL overlap)?
            if (freshStoryIds.has(existing.id))
                return false;
            if (existing.sources.some(src => freshSourceUrls.has(src.link)))
                return false;
            return true;
        })
            .map(s => ({
        ...
        Promise.allSettled(allStories.map(story => redis.set(`story:${story.id}`, story, { ex: STORY_ARCHIVE_TTL }))).catch(e => console.error('[CRON] Story archive write failed:', e));
...
//# sourceMappingURL=refresh-news.js.map