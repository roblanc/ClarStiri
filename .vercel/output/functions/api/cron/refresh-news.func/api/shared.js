import { BIAS_WEIGHT_MAP as SHARED_BIAS_WEIGHT_MAP, NEWS_SOURCES_BASE, } from '../shared/newsSources.js';
import { decodeHtmlEntities } from '../shared/htmlEntities.js';
export const NEWS_SOURCES = NEWS_SOURCES_BASE.map((source) => ({ ...source }));
export const BIAS_WEIGHT_MAP = SHARED_BIAS_WEIGHT_MAP;
 ...
 */
export function shouldFilterNews(title, description = '', url = '') {
    const text = `${title} ${description}`.toLowerCase();
    // URL-based filter — path segments are reliable signals
    if (url) {
        const urlLower = url.toLowerCase();
        if (FORBIDDEN_URL_SEGMENTS.some(seg => urlLower.includes(seg)))
            return true;
    }
...
export function quickBiasAnalysis(title, description = '') {
    const text = `${title} ${description}`.toLowerCase();
...
export async function fetchWithTimeout(url, timeout) {
    ...
    try {
        const response = await fetch(url, {
            signal: controller.signal, // type assertion for older node versions
            headers: {
...
function extractImageUrl(itemXml, rawDescription, rawContentEncoded) {
    const patterns = [
        /<enclosure[^>]+url=["']([^"']+)["']/i,
        /<media:content[^>]+url=["']([^"']+)["']/i,
...
export function parseRSSXML(xmlString, source) {
    ...
    while ((match = itemRegex.exec(xmlString)) !== null && index < MAX_ITEMS_PER_SOURCE) {
        ...
        const description = decodeHtmlEntities(rawDescription.replace(/<[^>]*>/g, '')).substring(0, 500);
 ...
 */
async function scrapeRealitateaNet(source) {
    try {
            ...
            try {
                ...
                for (const item of data.itemListElement) {
                    if (!item.url || !item.name)
                        continue;
                    if (shouldFilterNews(item.name, '', item.url))
                        continue;
...
export async function fetchRSSFeed(source, proxyIndex = -1) {
