...
function parseDomainInput(rawInput) {
...
function buildWaybackPatterns(rawInput) {
...
async function fetchWaybackCaptures(pattern) {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
    try {
        const apiUrl = `https://web.archive.org/cdx/search/cdx?url=${encodeURIComponent(pattern)}` +
