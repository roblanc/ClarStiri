...
function getFactualityScore(factuality) {
    if (factuality === 'high')
        return 3;
    if (factuality === 'mixed')
        return 2;
...
export async function getEmbeddingsBatch(texts) {
    if (!GROQ_API_KEY || texts.length === 0)
        return null;
    const controller = new AbortController();
...
export async function generateAggregatedTitle(articles) {
    const sortedByFactuality = [...articles].sort((a, b) => getFactualityScore(b.source.factuality) - getFactualityScore(a.source.factuality));
