...
function normalizeStatement(raw, index, fallbackUrl) {
    if (!isRecord(raw))
        return null;
...
async function enforceRateLimit(redis, req) {
    if (!redis)
        return true;
    try {
...
async function loadAugmentedStatements(redis, slug) {
    if (!redis || !slug)
        return [];
    try {
...
async function fetchInternalNews(name) {
...
async function generateStatementsWithGemini(name, articles) {
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const uniqueArticles = Array.from(new Map([...internalArticles, ...googleArticles].map((article) => [article.link, article])).values());
...
//# sourceMappingURL=analyze-voice.js.map