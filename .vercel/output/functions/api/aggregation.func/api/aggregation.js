import { BIAS_WEIGHT_MAP, shouldFilterNews } from './shared.js';
...
export async function resolveStoryImageFromSources(sources) {
    if (sources.length === 0)
        return undefined;
...
export function getTimeAgo(pubDate) {
    const now = new Date();
...
export function calculateBlindspot(bias, sourcesCount) {
    ...
    if (bias.left < 8 && (bias.right > 25))
        return 'left';
    // If Right is missing or very low while other sides are present
    if (bias.right < 8 && (bias.left > 25))
        return 'right';
    return 'none';
}
export function calculateBiasDistribution(sources) {
    if (sources.length === 0)
 ...
 */
async function getEmbeddingsSafe(titles) {
    const CHUNK_SIZE = 96;
    const allEmbeddings = [];
    for (let i = 0; i < titles.length; i += CHUNK_SIZE) {
        ...
        if (!result)
            return null; // dacă un chunk eșuează, dezactivăm semantic complet
        allEmbeddings.push(...result);
    }
    return allEmbeddings;
}
function filterRecentNews(news) {
    const cutoff = Date.now() - 7 * 24 * 60 * 60 * 1000; // 7 zile
    return news.filter(item => {
        const date = new Date(item.pubDate).getTime();
        if (!isNaN(date) && date <= cutoff)
            return false;
        if (shouldFilterNews(item.title, item.description ?? '', item.link))
            return false;
        return true;
    });
}
// Clusterizare avansată Ground News style cu semantic fallback
export async function findSimilarStories(news, threshold = 0.32, maxTimeDiffMs = 48 * 60 * 60 * 1000) {
    ...
    itemData.forEach((dataI, i) => {
        if (processed.has(dataI.item.id))
            return;
        const group = [dataI.item];
        processed.add(dataI.item.id);
        for (let j = i + 1; j < itemData.length; j++) {
            const dataJ = itemData[j];
            if (processed.has(dataJ.item.id))
                continue;
            if (Math.abs(dataI.time - dataJ.time) > maxTimeDiffMs)
                continue;
 ...
 */
function triggerWaybackArchive(url) {
    try {
...
export async function aggregateNewsBuildTopics(news, minSourcesParam = 3) {
    ...
    const aggregatedStoryTasks = [];
    const idCounts = new Map();
    let taskIndex = 0;
    storyGroups.forEach((sources) => {
        if (sources.length < minSourcesParam)
            return;
        ...
        const promise = async () => {
            ...
            const aggregatedTitle = useLlm
                ...
                : (sources.sort((a, b) => {
                    const order = { high: 2, mixed: 1, low: 0 };
