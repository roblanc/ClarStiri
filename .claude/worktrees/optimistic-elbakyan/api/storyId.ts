export interface StoryIdSource {
    title: string;
    pubDate: string;
}
...
function getEarliestDateKey(sources: StoryIdSource[]): string {
    const times = sources
...
function getTokenKey(sources: StoryIdSource[]): string {
...
function getRepresentativeTitle(sources: StoryIdSource[]): string {
    const counts = new Map<string, number>();
    sources.forEach(s => {
...
/**
 * Deterministic story id, stable across refreshes (as long as the grouped story stays similar).
 */
export function createStoryId(sources: StoryIdSource[]): string {
